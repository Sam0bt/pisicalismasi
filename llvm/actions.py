...
def setup():
    if not shelltools.isDirectory("tools/clang"):
        pisitools.dosed("tools/cfe-3.6.2.src/lib/Driver/ToolChains.cpp", '"ld"', '"ld.gold"')
...
def build():
    ...
    autotools.make("-C docs -f Makefile.sphinx man")
    autotools.make("make -C docs -f Makefile.sphinx html")
    autotools.make("-C tools/clang/docs -f Makefile.sphinx html")
